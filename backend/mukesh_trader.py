"""Mukesh Negi's custom Trader using a multi-specialist research team."""
...
class MukeshTrader(Trader):
    """Trader that consults Mukesh's research team before making a decision.
    ...
    async def create_agent(
        ...
        """Create Mukesh's Trader with the multi-specialist research team tool."""
    ...
    async def get_strategy(self) -> str:
        """Read Mukesh's strategy through the central account resource."""
    ...
    async def run_with_mcp_servers(self) -> str:
        """Open isolated tool bundles for Mukesh's Trader and specialists."""

        async with AsyncExitStack() as stack:
            trader_servers = [
                await stack.enter_async_context(server)
                for server in mukesh_trader_mcp_servers()
            ]
            researcher_servers = [
                await stack.enter_async_context(server)
